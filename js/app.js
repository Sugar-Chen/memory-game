...
var clickTime = 0;//记录移动次数--点击次数

var cardsArr = [];//存放所有卡片的数组
...
function clickEvents(){//循环遍历数组中的对象，给每个对象添加click事件，并执行操作
	for (var i = 0; i < cardsArr.length; i++) {
		cardsArr[i].onclick = clickFunc;
	}
}

function showAllCards(){//显示页面上的卡片
...
function createCardsArr(){//产生存放所有卡片的数组
...
function clickFunc(){//点击事件的函数
	...
	//被点击的卡片，显示符号和背景//在给当前图片添加背景和显示之前，先判断已经存在于数组中的元素是否和当前元素匹配
	showChar(this,(this.children)[0].className);
}

function moveTimes(){//显示用户移动次数
	clickTime++;
	moves.innerHTML = clickTime;
}

function showChar(t,curI){//显示当前卡片的符号，
	//被点击的卡片，显示符号和背景
	var curI = (t.children)[0].className;
	//在给当前图片添加背景和显示之前，先判断已经存在于数组中的元素是否和当前元素匹配
	for (var i = 0; i < openArr.length; i++) {
		if((openArr[i].children)[0].className == curI  ){
			openArr[i].className="card match";
			t.className = "card match";
			matchCard += 2;
			openArr.splice(i,1);
